...
if __name__ == '__main__':
    ...
    for submodule in module_list:
        ...
        for func in inspect.getmembers(submodule, inspect.isfunction):
            if func[0] in times:
                ...
                df.append({'module': submodule.__name__, 'function': func[0], 'time': times[func[0]]})
    ...
    fig = px.scatter(df, x='function', y='time', text='text', color='module', log_y=True)
    fig.update_xaxes(tickfont_size=8)
    fig.update_traces(textposition='top left')
    fig.update_layout(title_text=f'Execution time for a {l1}x{l1} image')
